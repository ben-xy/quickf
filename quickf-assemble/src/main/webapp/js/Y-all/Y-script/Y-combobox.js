...
define(function(){
	...
	Y.inherit('Combobox','box',{
	    doInit: function(cfg){   
			...
			var main = $('<input>').attr({type:'text',name: cfg.name}).addClass('select-input');
			var list = $('<ul>').css({position:'absolute',zIndex:6000,overflow:'hidden',display:'none'}).addClass('select-list');
			if(cfg.itemsHeight) {
			    list.css({height: cfg.itemsHeight,overflow:'auto'});
			}
			...
			$.each(store,function(i,item){
			    var aselect = $('<li>').addClass(item.css || cfg.itemCss).html(item.text);
		...
		doShow: function(speed){
			...
			if(cfg.autoFill) {
			   ...
			   this.main.css({width: this.combo.innerWidth() - this.selectObj.outerWidth(),height:'100%'});
			} else if(cfg.autoSize) {
			   setTimeout(function(){
			       _this.doSize();
			   },100);
			} else {
			    this.main.css({width: this.el.innerWidth()-this.selectObj.outerWidth(),height: this.el.innerHeight()-this.selectObj.outerHeight()});
			}
		},
		doSize: function(){
		    ...
		    this.getItemList().each(function(i,item){
				...
				if(strSize.height > maxSize.height) {
				    maxSize.height = strSize.height;
				}
			});
			if (maxSize.width != 0) {
			    if(this.cfg.maxWidth && this.cfg.maxWidth < maxSize.width + 6) {
				    this.main.css({width: this.cfg.maxWidth});
				} else {
				    this.main.css({width: maxSize.width + 6});
				}
			}
			this.combo.css({width:this.main.outerWidth()+this.selectObj.outerWidth(),height: this.main.outerHeight()});
		},
		showList: function(){
		...
		doSelect:function(param,type,noEv){
		    var cfg = this.cfg;
